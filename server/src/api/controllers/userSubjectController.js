...
class userSubjectController {
   async getAllUserSubjectsOfUserId(req, res, next) {
      try {
         const userSubjects = await UserSubject.findUserSubjects('user_id', req.params.id);
   ...
   async getAllUserSubjectsOfSubjectId(req, res, next) {
      try {
         const userSubjects = await UserSubject.findUserSubjects('subject_id', req.params.id);
         return res.status(200).json(userSubjects);
      } catch (error) {
         next(error);
      }
