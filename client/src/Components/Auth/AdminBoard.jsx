...
function AdminBoard() {
   ...
   const showAllSubjectsOfDepartment = async departmentId => {
      ...
      const res = await SubjectService.getAllsSubjectOfDepartmentId(departmentId, user.token);
      const data = res.data;
      setSubjects(data);
   };
   ...
   const showAllUsersOfDepartment = async departmentId => {
      ...
      const res = await UserService.getAllUsersOfDepartmentId(departmentId, user.token);
      const data = res.data;
      ...
      <>
         <details className="auth-session">
            <summary className="auth-session-title">
               Task management
               <div className="auth-session-title-right">Total: {postsUnaccept.length}</div>
            </summary>
            <div className="auth-session-body">
               ...
               {postsUnaccept?.map(post => (
                  <PostItem post={post} acceptPost={acceptPost} destroyPost={destroyPost} />
               ))}
            </div>
         </details>
         <details className="auth-session">
